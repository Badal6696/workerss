...
import { Search, MapPin, Star, ChevronRight, Shield, Award, Clock, Zap, CheckCircle } from "lucide-react";
        ...
        <div className="relative z-10 page-container text-center">
          <div className="animate-fade-in">
            ...
            <h1 className="font-display font-bold text-4xl md:text-6xl lg:text-7xl text-white mb-4 leading-tight text-balance">
              {heroLines[0]}
              <br />
              <span className="gold-text">{heroLines[1] || ""}</span>
            </h1>
              ...
              <div key={t.id} className="glass-card-hover p-6">
                <div className="flex gap-1 mb-3">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <Star key={i} size={14} fill={i < t.rating ? "#C9A84C" : "none"} className={i < t.rating ? "text-gold-500" : "text-ink-500"} />
                  ))}
                </div>
                <p className="text-gold-500 text-sm leading-relaxed mb-4 italic">"{t.text}"</p>
                <div className="flex items-center gap-3">
                  <img src={t.avatar} alt={t.name} className="w-10 h-10 rounded-full object-cover border border-gold-700/20" />
                  <div>
                    <div className="font-semibold text-gold-400 text-sm">{t.name}</div>
                    <div className="text-gold-700 text-xs">{t.city} • {t.service}</div>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="glass-card p-10 md:p-16 text-center relative overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <button onClick={() => navigate("/browse")} className="btn-gold px-8 py-4 text-base">
                  Find a Professional Now
                </button>
                <button onClick={() => navigate("/signup?role=worker")} className="btn-outline-gold px-8 py-4 text-base">
